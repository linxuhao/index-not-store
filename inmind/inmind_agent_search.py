#!/usr/bin/env python3
"""chain37 (pre-reg 2026-08-01): AGENTIC SEARCH on the canonical store — frozen base
+ text-protocol SEARCH tool (top-5/call, <=4 calls), then ANSWER. Leaderboard-eligible.
INMIND_SMOKE=1 -> 10 tasks. Out results/inmind/answers_agentsearch_{tag}.json"""
...
for i, t in enumerate(tasks):
    ...
    rows.append({"task_id": t["task_id"], "domain": t["domain"], "queries": queries,
                 ...
                 "arms": {"AGS": {"ans": answer, "ret": ctx}},
                 "gold_in": {"AGS": int(gold_hit_round > 0)}})
    print(f"[ags] {i:3d} searches={len(queries)} gold_round={gold_hit_round} "
          f"q={queries[:2]!r}", flush=True)

summary = {"n": len(rows),
           "gold_in": sum(r["gold_in"]["AGS"] for r in rows) / len(rows),
           "mean_searches": sum(r["n_searches"] for r in rows) / len(rows),
           "hit_round_hist": {k: sum(1 for r in rows if r["gold_hit_round"] == k)
                              for k in range(5)}}
json.dump({"summary": summary, "rows": rows},
          open(os.path.join(HERE, "results", "inmind", f"answers_agentsearch_{tag}.json"),
               "w"), indent=1)
