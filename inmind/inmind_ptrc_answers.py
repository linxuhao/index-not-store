#!/usr/bin/env python3
"""chain43c (pre-reg 2026-08-03): cash the two chain43b front-ends to answers.
Q_EMB  = embed the FROZEN base's user-voiced memory-check question (bb.q)
BA_EMB = embed the adapter's hop answer (ba.stmt)
Both: top-6 over the canonical store -> frozen-base cold answer (standard RAG
prompt). No training. Out results/inmind/answers_ptrc_arms.json"""
import json, os, sys
...
for i, t in enumerate(tasks):
    ...
    p = P[t["task_id"]]
    fronts = {"Q_EMB": p["BASE_RC"]["bb"]["q"] or t["query"],
              "BA_EMB": p["PT_RC"]["ba"]["stmt"] or t["query"]}
    ...
    rows.append(row)
    print(f"[43c] {i:3d} Q={row['gold_in']['Q_EMB']} BA={row['gold_in']['BA_EMB']}",
          flush=True)

summary = {"n": len(rows),
           "gold_in": {a: sum(r["gold_in"][a] for r in rows) / len(rows)
                       for a in ("Q_EMB", "BA_EMB")}}
json.dump({"summary": summary, "rows": rows},
          open(os.path.join(HERE, "results", "inmind", "answers_ptrc_arms.json"), "w"),
          indent=1)
