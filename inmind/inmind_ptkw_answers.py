#!/usr/bin/env python3
"""chain39: PT_KW end-to-end — per-task-adapter recall-tier keywords (from
pertask_ladder_full.json probeB) -> canonical top-6 retrieval -> base answer.
Leaderboard-eligible in-weight system. Out results/inmind/answers_ptkw.json"""
import json, os, sys
...
INJECT_AT = 41
kw = {r["task_id"]: r["probeB_recall"]["kw"] for r in
      json.load(open(os.path.join(HERE, "results", "inmind", "pertask_ladder_full.json")))["rows"]}
...
for i, t in enumerate(tasks):
    ...
    rows.append({"task_id": t["task_id"], "domain": t["domain"],
                 "arms": {"PT_KW": {"ans": ans, "ret": ret}},
                 "gold_in": {"PT_KW": int(gold_ln in ret)}})
    print(f"[ptkw] {i:3d} gold_in={rows[-1]['gold_in']['PT_KW']}", flush=True)

summary = {"n": len(rows),
           "gold_in": sum(r["gold_in"]["PT_KW"] for r in rows) / len(rows)}
json.dump({"summary": summary, "rows": rows},
          open(os.path.join(HERE, "results", "inmind", "answers_ptkw.json"), "w"),
          indent=1)
