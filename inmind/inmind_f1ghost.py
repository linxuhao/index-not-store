#!/usr/bin/env python3
"""chain46 (pre-reg 2026-08-06): F1 PARAPHRASE WRITES + GHOST DECAY TAIL.
...
for i, t in enumerate(tasks):
    ...
    for c in BG[INJECT_AT:INJECT_AT + N_BURY]:   # ghost tail: bury under FRESH chatter
        write_step(opt, "Note: " + c)
    alive, _ = bare_recall(t)
    cert1 = dlp_cert(t)
    ...
    rows.append({"task_id": t["task_id"], "domain": t["domain"],
                 ...
                 "after_bury": {"recall_alive": bool(alive),
                                ...
                                "scan_verbatim": sv1}})
    print(f"[f1] {i:3d} recall@{recall_steps} stmt_inst={sp['instance']} "
          f"scanV={sv['gold_rank']} scanP={hp['gold_rank']} | bury: "
          f"recall={int(alive)} cert={cert1:.3f} scanV={sv1['gold_rank']}", flush=True)
...
json.dump({"summary": summary, "rows": rows},
          open(os.path.join(HERE, "results", "inmind", f"f1ghost_{REGIME}_{tag}.json"), "w"),
          indent=1)
