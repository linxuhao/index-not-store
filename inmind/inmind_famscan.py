#!/usr/bin/env python3
"""chain45 (pre-reg 2026-08-04): RECOGNITION-TIER RETRIEVAL via FAMILIARITY SCAN.
...
def scan(t, base_gold_lp):
    """Rank 241 lines by dlogp; return gold rank + margin."""
    lines = BG[:INJECT_AT] + [t["user_message"]] + BG[INJECT_AT:]
...
for i, t in enumerate(tasks):
    ...
    for rnd in range(MAX_ROUNDS):
        for f in ("Note: " + t["user_message"], "CHATQA"):
            write_step(opt, f, t)
            steps += 1
        if scanA is None and dlp_cert(t) > CERT_TH:
            recog_steps = steps
            scanA = scan(t, base_gold_lp)
        ok, _ = bare_recall(t)
        if ok:
            recall_steps = steps
            break
    if scanA is None:
        recog_steps = steps
        scanA = scan(t, base_gold_lp)
    scanB = scan(t, base_gold_lp)
    rows.append({"task_id": t["task_id"], "domain": t["domain"],
                 "steps_recog": recog_steps, "steps_recall": recall_steps,
                 "scan_recognition": scanA, "scan_recall": scanB})
    print(f"[fam] {i:3d} recog@{recog_steps} recall@{recall_steps} "
          f"A: r={scanA['gold_rank']} d={scanA['gold_delta']} "
          f"B: r={scanB['gold_rank']} d={scanB['gold_delta']}", flush=True)
...
summary = {"n": len(rows),
           ...
           "scan_recognition": agg("scan_recognition"),
           "scan_recall": agg("scan_recall"),
           "stmt_column_reused_from_43b": {"recognition": 0.080, "recall": 0.136},
           "falsify_if_recognition_scan_le": 0.216}
json.dump({"summary": summary, "rows": rows},
          open(os.path.join(HERE, "results", "inmind", f"famscan_{tag}.json"), "w"),
          indent=1)
