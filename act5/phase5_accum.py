...
Run on rig:  HSA_OVERRIDE_GFX_VERSION=11.0.0 python phase5/phase5_accum.py --mech bf --seed 1234 --dev cuda:0
"""
...
ap.add_argument("--selftest-form", choices=["same", "question"], default="same")  # question: gate replay by an interrogative-form self-test, independent of the eval cloze (circularity control)
...
ap.add_argument("--save-adapter", action="store_true")   # save end-of-stream LoRA state to results/adapters/ (recovery probes)
ap.add_argument("--rank-probe", action="store_true")     # probe logs gold first-token rank per fact (queue 8 half-life ladder T2)
ap.add_argument("--qprobe", action="store_true")         # probe logs question-form qhits/qranks/qmargins (queue 10 generalized ladder)
ap.add_argument("--dtype", choices=["fp32", "bf16"], default="fp32")  # bf16: larger substrates (queue 11 ext); adds _Dbf16 tag
ap.add_argument("--jdirs", default=None)  # jlens_dirs.py cache -> per-probe J-projection (own/mismatch/rand) at layers 12,16,20,23 (CLAIMS queue 5 timeline)
...
def question_form(f):
    """Interrogative self-test form (surface-independent of the declarative eval cloze).
    'The user's favorite color is X.' -> 'Q: What is the user's favorite color?\\nA:'"""
    attr = f["statement"][len("The user's "): f["statement"].find(" is ")]
    return f"Q: What is the user's {attr}?\nA:"


@torch.no_grad()
def selftest_question(model, tok, facts):
    """Question-form hits for replay gating only (never scored as eval)."""
    model.eval(); hits = []
    for f in facts:
        ids = tok(question_form(f), return_tensors="pt").to(DEV)
...
def main():
    ...
    base = AutoModelForCausalLM.from_pretrained(
        args.model, torch_dtype=torch.bfloat16 if args.dtype == "bf16" else torch.float32,
    ...
    if args.firewall_n > 0:
        gs_items = L.load_gsm8k_subset(os.path.join(os.path.dirname(__file__), "..", "e0",
    ...
    for k, f in enumerate(facts):
        ...
        if (k + 1) % args.probe_every == 0 or k == len(facts) - 1:
            ...
            pt = {"k": k + 1, "hits": hits, "margins": margins}
            if args.rank_probe:  # gold first-token rank at answer slot (queue 8, T2 key-window tier)
                granks = []
                with torch.no_grad():
                    for f2 in facts[: k + 1]:
                        ids2 = tok(cloze(f2), return_tensors="pt").to(DEV)
                        lg = model(**ids2).logits[0, -1]
                        gf = tok(" " + f2["answer"], add_special_tokens=False)["input_ids"][0]
                        granks.append(int((torch.argsort(lg, descending=True) == gf).nonzero()[0]) + 1)
                pt["granks"] = granks
            if args.qprobe:  # question-form ladder tiers (queue 10): qhits/qranks/qmargins
                qhits, qranks, qmargins = [], [], []
                with torch.no_grad():
                    for f2 in facts[: k + 1]:
                        qp = question_form(f2)
                        ids2 = tok(qp, return_tensors="pt").to(DEV)
                        g2 = model.generate(**ids2, max_new_tokens=12, do_sample=False,
                                            pad_token_id=tok.pad_token_id)
                        comp = tok.decode(g2[0][ids2["input_ids"].shape[1]:],
                                          skip_special_tokens=True).split("\n")[0]
                        qhits.append(int(L.contains_match_ci(f2["answer"], comp)))
                        lg = model(**ids2).logits[0, -1]
                        gf = tok(" " + f2["answer"], add_special_tokens=False)["input_ids"][0]
                        qranks.append(int((torch.argsort(lg, descending=True) == gf).nonzero()[0]) + 1)
                        others = [x["answer"] for x in facts if x["fid"] != f2["fid"]]
                        dis = random.Random(31 * f2["fid"] + args.seed).choice(others)

                        def qlp(ans):
                            full = tok(qp + " " + ans + ".", return_tensors="pt").to(DEV)
                            npre = ids2["input_ids"].shape[1]
                            logits2 = model(**full).logits[0]
                            lp2 = torch.log_softmax(logits2[:-1], -1)
                            idsf = full["input_ids"][0]
                            return float(lp2[torch.arange(npre - 1, len(idsf) - 1), idsf[npre:]].sum())
                        qmargins.append(round(qlp(f2["answer"]) - qlp(dis), 2))
                pt["qhits"] = qhits
                pt["qranks"] = qranks
                pt["qmargins"] = qmargins
            if args.selftest_form == "question":
                st = selftest_question(model, tok, facts[: k + 1])
            ...
            pt.update({"cumrecall": round(cr, 3), "n_recalled": round(cr * (k + 1), 1),
                       "n_recognized": nrec})
            if args.jdirs:  # three-curve timeline: J-projection with nulls (CLAIMS queue 5)
                JD = getattr(main, "_jd", None)
                if JD is None:
                    JD = torch.load(args.jdirs, map_location="cpu"); main._jd = JD
                JLAY = [12, 16, 20, 23]
                jrows = []
                model.eval()
                with torch.no_grad():
                    for f in facts[: k + 1]:
                        ...
                        h = [hs[i + 1][0, -1].float().cpu() for i in JLAY]  # hs[0]=embeddings
                        v = JD["fact_toks"][f["fid"]]
                        vm = JD["fact_toks"][JD["derange"][f["fid"]]]
                        cs = torch.nn.functional.cosine_similarity
                        jrows.append({"fid": f["fid"],
                                      "own": [float(cs(h[a], JD["dirs"][v][i], dim=0)) for a, i in enumerate(JLAY)],
                                      "mism": [float(cs(h[a], JD["dirs"][vm][i], dim=0)) for a, i in enumerate(JLAY)],
                                      "rand": [float(cs(h[a], JD["rand"][v][i], dim=0)) for a, i in enumerate(JLAY)]})
                pt["jproj"] = {"layers": JLAY, "rows": jrows}
            curve.append(pt)
    ...
    out = {"mech": args.mech, "model": args.model, "n_stream": args.n_stream, "ws": args.ws, "rank": args.rank,
           ...
           "selftest_form": args.selftest_form, "facts_src": args.facts,
    ...
    if args.selftest_form != "same":
        tag += f"_ST{args.selftest_form}"
    ...
    if args.save_adapter:
        ad_dir = os.path.join(args.out, "adapters")
        os.makedirs(ad_dir, exist_ok=True)
        sd = {k: v.cpu() for k, v in model.state_dict().items() if "lora" in k.lower()}
        ap_fn = fn.replace(".json", ".pt")
        torch.save(sd, os.path.join(ad_dir, ap_fn))
        print(f"[accum] saved adapter {ap_fn} ({len(sd)} tensors)")
