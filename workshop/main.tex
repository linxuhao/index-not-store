...
climb, with a frozen instrument control at every step but the last. The accounting is
...
beats every trained front end we built but one; that one --- embed the adapter's
...
all, but its realistic write stream admits no frozen control, so the last step is
attributed to neither side. Around this climb we measure four properties of the write
...
under the same burial harness, natural facts survive at $0.92$--$0.95$ while pseudoword
facts fall to $0.15$ strict recall, yet every likelihood certificate survives with rising
...
but the last ships with an \emph{instrument control}: the identical pipeline with the write
removed, so each gain is attributed to memory or machinery before it is celebrated (the
final arm's realistic protocol admits no such control, and its step is attributed to
neither side). Second, every chain is pre-registered in a ledger with falsification lines;
...
the in-context ceiling ($0.448/0.84$ published; $0.56$ of our own measured $0.80$), with
the attribution honestly split between instrument and memory at every step but the last,
which is attributed to neither side (\S\ref{sec:climb}). (2) The certification--retrieval dose wall: recognition-style
...
persona-prompted direct question is answered from weights). Serving is always cold: the
...
(BGE-small embeddings; an asymmetric-instruction control leaves the query arm unchanged, so
its weakness is semantic, not an encoder artifact). End-to-end scores use the benchmark's
...
\bottomrule
\end{tabular}
\caption{The canonical program. Every write arm but the last has a frozen instrument
control; $n{=}125$ throughout; single judge (four shards). The $0.448$ headline carries
two caveats: the write stream also teaches the index the store's register, and no frozen
control exists under the real-stream protocol, so the last step is attributed to neither
side.}
\label{tab:ladder}
\end{table}
...
log cannot be question-indexed in advance. This frozen front end beats every trained
front end we built except one.
...
control --- consistent across the program but underpowered at $n{=}125$, sign test
$p{=}0.22$). Realism completes the climb: writing a 41-turn subset of the background
stream into the same adapter before the fact --- the deployment condition --- lifts the
simplest pipeline (state your best guess about the user; embed the sentence) to
$0.576/0.448$. Application of a retrieved fact decays
monotonically in context size once coverage is conditioned away
\citep{jin2025longrag,cuconasu2024noise}, so all arms return few, precise lines.
...
($125/125$ across the program) --- and nothing else works. The generative route is dead in
every cell of a $2{\times}2$ probe factorial. The discriminative route fails more
...
string ($0.632$) and nothing else: a held-out paraphrase scores $0.056$
...
regimes, certificates $1.00$ with rising margins (an $n{=}8$ smoke with same-register
fact burial replicates the survival). Rerunning the identical harness with unguessable
pseudoword facts (``My transit code is roptavvor'') collapses recall to $0.15$ (strict
value production) --- while every certificate survives, margins \emph{rising} from $3.8$ to
...
here is \emph{lexical/entity familiarity} under a matched template and burial harness ---
...
weights know helps only by changing what is retrieved into context; in-weight generation
at serve time contributed zero everywhere it was isolated.
...
an agentic reader is untested), so it matters most where latency or search budgets bind. (3) \emph{Certification is not retrieval}: a membership-style
certificate at two steps proves presence, not findability; budget the recall dose if the
weights must be searched. (4) \emph{The write format chooses the key}; paraphrase writes
...
(noise anchored at $1/90$); the $+0.048$ margin is a consistent direction, not a finding;
the real-stream gain partly reflects the index learning the register of the very store it
retrieves from; the content axis is demonstrated under one fixed harness (write form and
