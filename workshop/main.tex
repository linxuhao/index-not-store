...
the best frozen front end ($32{:}15$ discordant tasks, exact $p{=}0.019$), with the
pipeline's own write-free controls (no writes; chatter-only writes) collapsing to
$0.080$, every statement a refusal. Around this climb we measure four properties of the write
...
real-stream arm carries two such controls --- no writes, and chatter-only writes --- both
of which collapse to $0.080$). Second, every chain is pre-registered in a ledger with falsification lines;
...
\citep{song2026beyondppl}. The certificate is not a curiosity of this rig: in the
companion program (anonymized supplementary) the same adapter-versus-base signal
separates written from never-written content at AUC $0.93$--$0.99$ with the pre-write
base at chance by construction, and recognition survives recall death across mechanisms
--- a matched-budget editing arm recalls $0$--$1/48$ while recognizing $45$--$47/48$.
...
Real-stream controls (no writes / chatter-only) & none & $0.080$ & $0.080$ \\
...
write's ($+0.136$ over the best frozen front end, exact $p{=}0.019$); the register
assist the write stream provides cashes only conditional on the fact being written.}
\label{tab:ladder}
\end{table}
...
stream into the same adapter before the fact, and ten further turns after it --- the
deployment condition --- lifts the simplest pipeline (state your best guess about the
user; embed the sentence) to $0.576/0.448$; the post-fact writes are load-bearing
(\S\ref{sec:wall}). And here, finally, the write itself is: two controls run the
identical statement pipeline with the gold write removed --- once with no writes at all,
once with the 51 chatter writes but no gold --- and both collapse to $0.080$ retrieval /
$0.080$ application, every statement the refusal ``I don't remember'' ($125/125$,
byte-identical across the two controls). Register-learning without the fact does not
even reach the embedding stage; the last step belongs to the write. Scope this
correctly: frozen retrieval is not what collapsed --- the frozen question front end
still stands at $0.384/0.312$, and log-only memory harnesses work; what the controls
kill is the register-echo reading of \emph{this} pipeline, whose statement form works
only written (frozen statement $0.080$ vs frozen question $0.384$). The write's cashed
end-to-end value, measured against the best frozen front end, is $+0.136$ ($32{:}15$
discordant tasks, exact McNemar $p{=}0.019$; computed post hoc on the paired judged
outcomes --- the pre-registered lines were the control collapses). The frozen maximum
$0.312$ is shared by a budget-8 frozen-seed agentic arm, and the margin survives that
comparator too ($35{:}18$, $p{=}0.027$). Application of a retrieved fact decays
...
fact burial replicates the survival). Rerunning the write-and-burial harness with unguessable
...
the frozen base ($0.136$ vs $0.216$), and consolidating from the model's own outputs
lands below doing nothing ($4.3$ vs $25.0$ of $144$) --- serving cold is load-bearing,
not stylistic.
...
and there it is largest: the write's retrieval edge concentrates at small $k$
(rank-curves from the same artifacts, post hoc: written statement $0.520$ vs frozen
question $0.248$ at $k{=}1$, a $2.1\times$ margin that shrinks to $1.3\times$ by
$k{=}20$; the written front end at $k{=}1$ beats the frozen one at $k{=}20$). The write
...
replay, which reaches the recall dose \emph{across} turns --- maintains a 384-fact stream
